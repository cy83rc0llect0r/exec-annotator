...
func annotatePod(namespace, podName, username string) error {
	...
	currentUser, exists := pod.Annotations["exec-user"]
	if !exists || currentUser == username {
		pod.Annotations["exec-user"] = username
		pod.Annotations["exec-time"] = time.Now().UTC().Format(time.RFC3339)
	} else {
		log.Infof("Skipping annotation update for %s/%s: different user (%s vs %s)", namespace, podName, username, currentUser)
		return nil
	}
	...
	log.Infof("Updated annotations on pod %s/%s: exec-user=%s, exec-time=%s", namespace, podName, username, pod.Annotations["exec-time"])
	return nil
}
